...
export class Equipment {
  ...
  constructor(data: any[], language: AssetFinderOptions["language"]) {
    this.artifacts = data.find(x => x.flat.itemType == 'ITEM_RELIQUARY') ? data.filter(x => x.flat.itemType == 'ITEM_RELIQUARY').map((data) => new Artifact(data, language)) : []
    this.weapon = new Weapon(data.filter(x => x.flat.itemType == 'ITEM_WEAPON')[0], language)
  }
}