...
import { HoyoBuilds, Hoyos, Owner } from "../Enka";
...
export class PlayerData {
  ...
  constructor(
    ...
    this.player = data.playerInfo
      ? new Player(data.playerInfo, this.language)
      : ({} as Player);
  ...
  async getHoyo(): Promise<Hoyos> {
    ...
    const data = await this.handler.profile(
      `${this.owner.username}/hoyos/${this.owner.hash}`
    );

    return new Hoyos(data, this.language, this.owner.username);
  }
  ...
  async getHoyoBuilds(): Promise<HoyoBuilds[]> {
    ...
    const data = await this.handler.profile(
      `${this.owner.username}/hoyos/${this.owner.hash}/builds`
    );
